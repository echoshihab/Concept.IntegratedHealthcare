...
namespace Proto.PatientRecordSystem.Controllers.Domain
{
    public class PatientController : DomainControllerBase<PatientDto>
    {
        ...
        [HttpPut("{mrn}")]
        public override async Task<IActionResult> UpdateAsync(string mrn, PatientDto resource)
        {
            return this.Ok(await this._domainResourceService.UpdateAsync(mrn, resource));
        }
